...
*Note:* `dsync` is an alias for `distro-sync`. Fedora users may be used to using `dnf up`, OpenMandriva is different. For Cooker and ROME installations it is strongly recommended to use `dnf dsync`. 
